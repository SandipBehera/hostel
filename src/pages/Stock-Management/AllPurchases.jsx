...
const AllPurchases = () => {
  ...
  const calculateTotalPrice = (purchase) => {
    return purchase.items.reduce((total, item) => total + parseInt(item.price), 0);
  };
        ...
        <Modal isOpen={modal} toggle={toggleModal}>
          ...
          <ModalBody>
            <div className='d-flex justify-content-between'>
              <div>
                <p> Purchased From: {currentPurchase?.purchasedBy}</p> <p>Dated: {`${new Date(currentPurchase?.date)}`.slice(4, 15)}</p>
              </div>
            ...
            <p className='pl-3 total-price'>Total Price: {currentPurchase && calculateTotalPrice(currentPurchase)}/-</p>
          </ModalBody>
          <ModalFooter>
